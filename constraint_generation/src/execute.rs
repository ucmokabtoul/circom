use type_analysis::analyzers::{*, cfg::PathAnalysisLint, lint::{LintLevel}, termination::TerminationAnalyser, termination::is_for_loop};
...
enum ExecutionWarning {
    ...
    UnassignedSignalOnExecutionPath(PathAnalysisLint),
}
...
pub fn constraint_execution(
    ...
    let folded_value_result = if let Call { id, args, .. } = &program_archive.get_main_expression()
    {
        ...
        let mut path_analyser = PathAnalyser::new(program_archive);
...
pub fn execute_constant_expression(
    ...
    flags: FlagsExecution,
    prime: &String,
    path_analyser: &mut PathAnalyser,
    ...
    let folded_value_result =
        execute_expression(expression, program_archive, &mut runtime_information, flags, path_analyser);
    match folded_value_result {
        Result::Err(_) => Result::Err(runtime_information.runtime_errors),
        Result::Ok(folded_value) => {
    ...
    let res = match stmt {
            ...
            match xtype {
                ...
                _ => {
                    ...
                    for dimension in dimensions.iter() {
                        let f_dimensions =
                            execute_expression(dimension, program_archive, runtime, flags, path_analyser)?;
        ...
        Substitution { meta, var, access, op, rhe, .. } => {
            let access_information =
                treat_accessing(meta, access, program_archive, runtime, flags, path_analyser)?;
            let r_folded = execute_expression(rhe, program_archive, runtime, flags, path_analyser)?;
        ...
        ConstraintEquality { meta, lhe, rhe, .. } => {
            ...
            let f_left = execute_expression(lhe, program_archive, runtime, flags, path_analyser)?;
            let f_right = execute_expression(rhe, program_archive, runtime, flags, path_analyser)?;
        ...
        Return { value, .. } => {
            let mut f_return = execute_expression(value, program_archive, runtime, flags, path_analyser)?;
        ...
        Block { stmts, .. } => {
            ...
            //NOTE: (@okabtoul): BEGIN Termination analysis
            if !path_analyser.checked_for_loops.contains(&stmt.get_meta().elem_id) {
                path_analyser.checked_for_loops.insert(stmt.get_meta().elem_id);
                if let Some(for_loop) = is_for_loop(program_archive, stmts) {
                    let mut termination_analyser = TerminationAnalyser::new(Some(for_loop));
                    let mut reports = ReportCollection::new();
                    let lints = termination_analyser.analyse(program_archive);
                    for lint in lints {
                        let mut report = match (lint.error_code, lint.level) {
                            (ReportCode::AnonymousCompLint, _) => Report::note(lint.error_msg, lint.error_code),
                            (ReportCode::ConstantSignalLint, _) => Report::note(lint.error_msg, lint.error_code),
                            (ReportCode::LoopNoProgress, LintLevel::Error) => Report::error(lint.error_msg, lint.error_code),
                            (ReportCode::LoopNoProgress, _) => Report::warning(lint.error_msg, lint.error_code),
                            (ReportCode::LoopMayOverflow, LintLevel::Error) => Report::error(lint.error_msg, lint.error_code),
                            (ReportCode::LoopMayOverflow, _) => Report::warning(lint.error_msg, lint.error_code),
                            _ => unreachable!(""),
                        };
                        report.add_primary(lint.loc, *program_archive.get_file_id_main(), lint.msg);
                        reports.push(report);
                    }
                    if !reports.is_empty() {
                        for report in reports.iter() {
                            add_report_to_runtime(report.clone(), stmt.get_meta(), &mut runtime.runtime_errors, &runtime.call_trace, None);
                        }
                        if reports.iter().find(|r| r.is_error()).is_some() {
                            return Result::Err(());
                        }
                    }
                }
            }
            //NOTE: (@okabtoul): END Termination analysis
            ...
            if flags.verbose {
                ...
                for arglog in args {
                    if let LogArgument::LogExp(arg) = arglog {
                        let f_result = execute_expression(arg, program_archive, runtime, flags, path_analyser)?;
        ...
        UnderscoreSubstitution { meta, rhe, op } => {
            let f_result = execute_expression(rhe, program_archive, runtime, flags, path_analyser)?;
    ...
    let res = match expr {
        ...
        Variable { meta, name, access, .. } => {
            if ExecutionEnvironment::has_signal(&runtime.environment, name) {
                //TODO: check signal
                execute_signal(meta, name, access, program_archive, runtime, flags, path_analyser)?
            } else if ExecutionEnvironment::has_component(&runtime.environment, name) {
                execute_component(meta, name, access, program_archive, runtime, flags, path_analyser)?
            } else if ExecutionEnvironment::has_variable(&runtime.environment, name) {
                execute_variable(meta, name, access, program_archive, runtime, flags, path_analyser)?
            } else {
                unreachable!();
            }
        }
        ArrayInLine { meta, values, .. } => {
            ...
            for value in values.iter() {
                let f_value = execute_expression(value, program_archive, runtime, flags, path_analyser)?;
        ...
        UniformArray { meta, value, dimension, .. } => {
            let f_dimension = execute_expression(dimension, program_archive, runtime, flags, path_analyser)?;
        ...
        InfixOp { meta, lhe, infix_op, rhe, .. } => {
            let l_fold = execute_expression(lhe, program_archive, runtime, flags, path_analyser)?;
            let r_fold = execute_expression(rhe, program_archive, runtime, flags, path_analyser)?;
        ...
        PrefixOp { prefix_op, rhe, .. } => {
            let folded_value = execute_expression(rhe, program_archive, runtime, flags, path_analyser)?;
        ...
        InlineSwitchOp { cond, if_true, if_false, .. } => {
            let f_cond = execute_expression(cond, program_archive, runtime, flags, path_analyser)?;
            ...
            if let Option::Some(bool_cond) = possible_bool_cond {
                if bool_cond {
                    execute_expression(if_true, program_archive, runtime, flags, path_analyser)?
                } else {
                    execute_expression(if_false, program_archive, runtime, flags, path_analyser)?
                }
            } else {
        ...
        Call { id, args, .. } => {
            let (value, can_simplify) = execute_call(id, args, program_archive, runtime, flags, path_analyser)?;
            can_be_simplified = can_simplify;
            value
        }
        ParallelOp { rhe, .. } => {
...
fn execute_call(
    ...
    let mut arg_values = Vec::new();
    for arg_expression in args.iter() {
        let f_arg = execute_expression(arg_expression, program_archive, runtime, flags, path_analyser)?;
    ...
    if program_archive.contains_function(id) {
        ...
        let folded_result = execute_function_call(id, program_archive, runtime, flags, path_analyser)?;
...
fn execute_template_call_complete(
    ...
    if program_archive.contains_template(id) {
        ...
        runtime.call_trace.push(id.clone());
        let folded_result =
            execute_template_call(id, arg_values, tags, program_archive, runtime, flags, path_analyser)?;
            ...
            */
            ...
            for i in 0..SignalSlice::get_number_of_cells(&signal_previous_value) {
                ...
                if signal_was_assigned {
                    //NOTE (@okabtoul): Double-assignment analysis
                    if let AssignOp::AssignSignal|AssignOp::AssignConstraintSignal = op {
                        let template_name = runtime.call_trace.last().unwrap();
                        if path_analyser.constraint_signal_assignment(template_name, meta, &full_symbol).is_err() {
...
fn execute_conditional_statement(
    ...
    let f_cond = execute_expression(condition, program_archive, runtime, flags, path_analyser)?;
    let ae_cond = safe_unwrap_to_single_arithmetic_expression(f_cond, line!());
    ...
    if let Some(cond_bool_value) = possible_cond_bool_value {
        let (ret_value, can_simplify) = match false_case {
            Some(else_stmt) if !cond_bool_value => {
                execute_statement(else_stmt, program_archive, runtime, actual_node, flags, path_analyser)?
            }
            None if !cond_bool_value => (None, true),
            _ => execute_statement(true_case, program_archive, runtime, actual_node, flags, path_analyser)?,
        };
        ...
        let (mut ret_value, mut can_simplify) =
            execute_statement(true_case, program_archive, runtime, actual_node, flags, path_analyser)?;
...
fn execute_sequence_of_statements(
    ...
    for stmt in stmts.iter() {
        let (f_value, can_simplify) =
            execute_statement(stmt, program_archive, runtime, actual_node, flags, path_analyser)?;
        can_be_simplified &= can_simplify;
        if f_value.is_some() {
    ...
    if is_complete_template {
        execute_delayed_declarations(program_archive, runtime, actual_node, flags, path_analyser)?;
    }
...
fn execute_delayed_declarations(
    ...
    for (component_name, (meta, dimensions)) in runtime.anonymous_components.clone() {
        let mut arithmetic_values = Vec::new();
        for dimension in dimensions.iter() {
            let f_dimensions = execute_expression(dimension, program_archive, runtime, flags, path_analyser)?;
            arithmetic_values
...
fn execute_variable(
    ...
    let access_information = treat_accessing(meta, access, program_archive, runtime, flags, path_analyser)?;
...
fn execute_signal(
    ...
    //FIXME: check template contained in program
    let template_name = runtime.call_trace.last().unwrap();
    //TODO: need to report at end to not show duplicates.
    if let Some(lint) = lint_all_paths_to_signal(&program_archive, template_name, symbol, meta) {
            let err: Result<(), ExecutionWarning> = Err(ExecutionWarning::UnassignedSignalOnExecutionPath(lint));
            treat_result_with_execution_warning(err, meta, &mut runtime.runtime_errors, &runtime.call_trace)?;
    }
    let access_information = treat_accessing(meta, access, program_archive, runtime, flags, path_analyser)?;
    if access_information.undefined {
        let arithmetic_slice = Option::Some(AExpressionSlice::new(&AExpr::NonQuadratic));
    ...
    loop {
        ...
        match &access[act] {
            Access::ArrayAccess(index) => {
                let index_fold = execute_expression(index, program_archive, runtime, flags, path_analyser)?;
...
fn treat_accessing(
    ...
    let (ae_before_signal, signal_name, signal_index) =
        treat_indexing(0, access, program_archive, runtime, flags, path_analyser)?;
    let (ae_after_signal, tag_name, _tag_index) =
        treat_indexing(signal_index + 1, access, program_archive, runtime, flags, path_analyser)?;
    treat_result_with_memory_error(
...
fn treat_result_with_arithmetic_error<C>(
    ...
    match arithmetic_error {
        ...
        Result::Err(arithmetic_error) => {
            ...
            add_report_to_runtime(report, meta, runtime_errors, call_trace, None);
            Result::Err(())
        }
    }
}
...
fn treat_result_with_execution_warning<C>(
    ...
    match execution_error {
        ...
        Result::Err(execution_warning) => {
            let (report, secondary_message) = match execution_warning {
                CanBeQuadraticConstraintSingle() => {
                    let msg = format!(
                    ...
                    (Report::warning(msg, ReportCode::RuntimeWarning), None)
                }
                ...
                UnassignedSignalOnExecutionPath(lint) => {
                    (Report::warning(format!("Unassigned signal `{}`", lint.name), ReportCode::UnassignedSignalOnExecutionPath), Some(lint.msg))
                }
            };
            add_report_to_runtime(report, meta, runtime_errors, call_trace, secondary_message);
            Result::Ok(())
        }
    }
}
...
fn add_report_to_runtime(
    ...
    if let Some(message) = secondary_message {
        report.add_secondary(meta.location.clone(), meta.get_file_id(), Some(message.to_owned()));
    }
